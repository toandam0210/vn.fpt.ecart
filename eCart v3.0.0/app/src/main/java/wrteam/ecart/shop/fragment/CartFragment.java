...
public class CartFragment extends Fragment {
    ...
    public static List<ItemInCart> carts;
                    ...
                    if (Float.parseFloat(session.getData(Constant.min_order_amount)) <= Constant.FLOAT_TOTAL_AMOUNT) {
                        if (session.getBoolean(Constant.IS_USER_LOGIN)) {
                            if (rgOrderType.getCheckedRadioButtonId() == R.id.rbDoorStepDelivery) {
                                ...
                                bundle.putSerializable("data", (Serializable) carts);
    ...
    private void getCartData() {
        ...
        ApiConfig.RequestToVolley((result, response) -> {
            if (result) {
                try {
                    CartService cartService1 = db.cartService();
                    Cart cart = cartService1.loadCartById(Integer.valueOf(session.getData(Constant.USER_ID)));

                    CartItemsService cartItemsService = db.cartItemsService();
                    List<CartItems> cartItems = cartItemsService.loadCartById(cart.getId());
                    ...
                    String taxPercentage = cartItems.get(0).getTax_percentage();

                    if (cartItems.get(0).getDiscounted_price().equals("0") || cartItems.get(0).getDiscounted_price().equals("")) {
                        price = ((Float.parseFloat(cartItems.get(0).getPrice()) + ((Float.parseFloat(cartItems.get(0).getPrice()) * Float.parseFloat(taxPercentage)) / 100)));
                    } else {
                        price = ((Float.parseFloat(cartItems.get(0).getDiscounted_price()) + ((Float.parseFloat(cartItems.get(0).getDiscounted_price()) * Float.parseFloat(taxPercentage)) / 100)));
                    }
